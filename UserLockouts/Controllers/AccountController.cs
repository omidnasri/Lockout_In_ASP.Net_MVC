...
namespace UserLockouts.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            // یافتن کاربر با استفاده از ایمیل وارد شده
            var user = await UserManager.FindByEmailAsync(model.Email);
            // در صورتی که کاربری پیدا یافت نشود صفحه + مدل مجدد برگشت داده میشود
            if (user == null) return View(model);
            //
            var validCredentials = await UserManager.FindAsync(model.Email, model.Password);

            // در صورت غیرفعال بودن حساب پیغامی برگشت داده میشود
            if (await UserManager.IsLockedOutAsync(user.Id))
                ModelState.AddModelError("", "حساب شما غیر فعال میباشد، بعد از 15 دقیقه دیگر مجدد تلاش کنید");
            ...
            else if (await UserManager.GetLockoutEnabledAsync(user.Id) && validCredentials == null)
            {
                ...
                string message;
                // برسی میشود حساب کاربری فعال است یا خیر
                if (await UserManager.IsLockedOutAsync(user.Id))
                {
            ...
            else
            {
                ...
                // ریست کردن فیلد تعداد دفعات اشتباه
                await UserManager.ResetAccessFailedCountAsync(user.Id);
                return RedirectToLocal(returnUrl);
            }
            // در صورت بروز خطا این خط فراخوانی میشود
            return View(model);
        }
